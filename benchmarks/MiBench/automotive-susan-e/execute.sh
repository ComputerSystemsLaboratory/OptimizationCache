...
function verify_output() {
    # Diff the two files.
    diff reference_output/reference_output_$WORKING_SET.pgm output.pgm > diff.txt 2>&1
    if [[ $? -eq 0 ]]; then
        # They are igual
...
case $WORKING_SET in
    0)
        RUN_OPTIONS="input_small.pgm output.pgm -e"
        ;;
    1)
        RUN_OPTIONS="input_large.pgm output.pgm -e"
        ;;
    *)
        echo "Error: dataset"
        exit 1

...
{	int i, j;
	dprintf_3("[L]width=%d, height=%d, raster=%d\n",
		 width, height, raster);
	for ( i = 0; i < height; i++ )
...
{	gx_device *target = cdev->target;
	   ...
	   {	int band_height = cdev->page_band_height;
		...
		for ( i = 0; i < num_pages; ++i )
		  { const gx_placed_page *ppage = &ppages[i];
		    ...
		    if ( (rs.page_cfile != 0 ||
			  (code = clist_fopen(rs.page_cfname, gp_fmode_rb,
					      &rs.page_cfile, mem, true)) >= 0) &&
			 (rs.page_bfile != 0 ||
			  (code = clist_fopen(rs.page_bfname, gp_fmode_rb,
					      &rs.page_bfile, mem, false)) >= 0)
		       )
		      { stream s;
		        byte sbuf[cbuf_size];
			...
			code = clist_render_band(cdev, &s, tdev,
						 -ppage->offset.x,
						 band * mdev.height, mem);
		      }
		    /* Close the files if we just opened them. */
		    if ( ppage->page->info.bfile == 0 && rs.page_bfile != 0 )
		      clist_fclose(rs.page_bfile, rs.page_bfname, false);
		    if ( ppage->page->info.cfile == 0 && rs.page_cfile != 0 )
	...
	{ byte *src = mdata + (y - cdev->ymin) * raster;
	  if ( actual_data == 0 )
  ...
  if ( cb_limit > cb_end ) cb_limit = cb_end
		...
		if ( gs_debug_c('L') )
		  { const char **sub = cmd_sub_op_names[op >> 4];
		    if ( sub )
		      dprintf_1("[L]%s:", sub[op & 0xf]);
		    else
		      dprintf_2("[L]%s %d:", cmd_op_names[op >> 4], op & 0xf);
		  }
#endif
		switch ( op >> 4 )
		   {
				...
				{ int n = *cbp++;
				  if ( ht_data_index < porder->num_levels )
					...
					(porder->levels + ht_data_index);
				      ...
				      if ( gs_debug_c('L') )
					{ int i;
					  dprintf_1(" levels[%u]", ht_data_index);
					  for ( i = 0; i < n; ++i )
					    dprintf_1(" %u",
					...
					(porder->bits +
					 (ht_data_index - porder->num_levels));
				      ...
				      if ( gs_debug_c('L') )
					{ int i;
					  dprintf_1(" bits[%u]", ht_data_index - porder->num_levels);
					  for ( i = 0; i < n; ++i )
					    { const gx_ht_bit *pb =
		...
		case cmd_op_misc2 >> 4:
			...
			case cmd_opv_set_dash:
				{ int nb = *cbp++;
				  ...
				  if ( gs_debug_c('L') )
				    { int i;
				      dprintf_4(" dot=%g(mode %d) adapt=%d offset=%g [",
					       dot_length,
					       (nb & 0x40) != 0,
					       (nb & 0x80) != 0, offset);
				      for ( i = 0; i < n; ++i )
					dprintf_1("%g ", dash_pattern[i]);
				      dputs("]\n");
				    }
#endif
				  cbp += n * sizeof(float);
				...
				{ byte b = *cbp++;
				  ...
				  code = (*dev_proc(tdev, begin_image))
				    (tdev, &imager_state, &image,
				     gs_image_format_chunky, &image_rect,
				...
				{ uint height;
				  ...
				  if ( height == 0 )
					...
					cmd_print_bits(data, image_rect.q.x -
						       image_rect.p.x, height,
						       bytes_per_row);
#endif
				      code = (*dev_proc(tdev, image_data))
					(tdev, image_info, &data, data_x,
...
bad_op:			lprintf5("Bad op %02x band y0 = %d file pos %ld buf pos %d/%d\n",
			   ...
			   {	const byte *pp;
				for ( pp = cbuf; pp < cb_end; pp += 10 )
				  { dprintf_1("%4d:", (int)(pp - cbuf));
				    dprintf_10(" %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x\n",

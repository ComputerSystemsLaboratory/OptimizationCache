...
{	unsigned size = r_size(pref);
	ref nref;
	dprintf_1("(%x)", r_type_attrs(pref));
	switch ( r_type(pref) )
	   {
	case t_array:
	  dprintf_2("array(%u)0x%lx", size, (ulong)pref->value.refs); break;
	case t_astruct:
	  goto strct;
	case t_boolean:
	  dprintf_1("boolean %x", pref->value.boolval); break;
	case t_device:
	  dprintf_1("device 0x%lx", (ulong)pref->value.pdevice); break;
	case t_dictionary:
	  dprintf_3("dict(%u/%u)0x%lx",
		   dict_length(pref), dict_maxlength(pref),
		   (ulong)pref->value.pdict);
	  break;
	case t_file:
	  dprintf_1("file 0x%lx", (ulong)pref->value.pfile); break;
	case t_fontID:
	  goto strct;
	case t_integer:
	  dprintf_1("int %ld", pref->value.intval); break;
	case t_mark:
	  dprintf_("mark"); break;
	case t_mixedarray:
	  dprintf_2("mixed packedarray(%u)0x%lx", size,
		   (ulong)pref->value.packed); break;
	case t_name:
	  dprintf_2("name(0x%lx#%u)", (ulong)pref->value.pname,
		   name_index(pref));
	...
	case t_null:
	  dprintf_("null"); break;
	case t_oparray:
	  dprintf_2("op_array(%u)0x%lx:", size, (ulong)pref->value.const_refs);
	...
	case t_operator:
	  ...
	  if ( size > 0 && size < op_def_count ) /* just in case */
	    dprintf_1(":%s", (const char *)(op_def_table[size]->oname + 1));
	  dprintf_1(")0x%lx", (ulong)pref->value.opproc);
	  break;
	case t_real:
	  dprintf_1("real %f", pref->value.realval); break;
	case t_save:
	  dprintf_1("save %lu", pref->value.saveid); break;
	case t_shortarray:
	  dprintf_2("short packedarray(%u)0x%lx", size,
		   (ulong)pref->value.packed); break;
	case t_string:
	  dprintf_2("string(%u)0x%lx", size, (ulong)pref->value.bytes); break;
	case t_struct:
strct:	  { obj_header_t *obj = (obj_header_t *)pref->value.pstruct;
	    dprintf_2("struct %s 0x%lx",
		     (r_is_foreign(pref) ? "-foreign-" :
		      gs_struct_type_name_string(gs_object_type(imemory, obj))),
		     (ulong)obj);
	  }
	  break;
	default: dprintf_1("type 0x%x", r_type(pref));
	   }
}
private void
...
{	ushort elt = *pref & packed_value_mask;
	...
	case pt_executable_operator:
	  ...
	  break;
	case pt_integer:
	  dprintf_1("<int> %d", (int)elt + packed_min_intval);
	  break;
	case pt_literal_name:
	  dprintf_("<lit_name>"); goto ptn;
	case pt_executable_name:
	  dprintf_("<exec_name>");
ptn:	  name_index_ref(elt, &nref);
	  dprintf_2("(0x%lx#%u)", (ulong)nref.value.pname, elt);
	  debug_print_name(&nref);
	  break;
	default:
	  dprintf_2("<packed_%d?>0x%x", *pref >> r_packed_type_shift, elt);
	}
}
void
...
#define buf_size 30
	...
	if ( type >= tx_next_index )
	  dprintf_1("0x%02x?? ", type);
	else if ( type >= t_next_index )
	  dprintf_("opr* ");
	else
	  dprintf_1("%s ", type_strings[type]);
	for ( ; ap->mask; ++ap )
	  if ( (attrs & ap->mask) == ap->value )
	    dputc(ap->print);
	dprintf_2(" 0x%04x 0x%08lx", r_size(p), *(const ulong *)&p->value);
	...
	for ( i = ref_stack_count(pstack); i != 0; )
	  { const ref *p = ref_stack_index(pstack, --i);
	    if ( m )
	      dprintf_2("%s at 0x%lx:\n", m, (ulong)pstack),
	      m = NULL;
	    dprintf_2("0x%lx: 0x%02x ", (ulong)p, r_type(p));
	    debug_dump_one_ref(p);
	    dputc('\n');
	  }
}
...
{	const ref_packed *pp;
	...
	switch (type)
	   {
	default:
		dprintf_2 ("%s at 0x%lx isn't an array.\n",

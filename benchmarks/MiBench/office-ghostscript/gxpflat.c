...
{	fixed
	...
	{	const fixed
		...
		uint q = (d - (d >> 2) /* 3/4 * D */ + fixed_flat - 1) /
		  fixed_flat;
...
#ifdef DEBUG
	if ( gs_debug_c('3') )
	  dprintf_4("[3]x0=%f y0=%f x1=%f y1=%f\n",
		   fixed2float(x0), fixed2float(y0),
		   fixed2float(x1), fixed2float(y1)),
	  dprintf_5("   x2=%f y2=%f x3=%f y3=%f  k=%d\n",
...
#ifdef DEBUG
		if ( gs_debug_c('3') )
		  dprintf_4("[3]dx=%f+%d, dy=%f+%d\n",
			   fixed2float(idx), rdx,
			   fixed2float(idy), rdy),
		  dprintf_4("   d2x=%f+%d, d2y=%f+%d\n",
			   fixed2float(id2x), rd2x,
			   fixed2float(id2y), rd2y),
		  dprintf_4("   d3x=%f+%d, d3y=%f+%d\n",

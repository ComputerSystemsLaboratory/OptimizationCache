...
{	byte *ptr = 0;
	...
	if ( size > gs_malloc_limit - sizeof(malloc_block)
	  ...
	  { uint added = size + sizeof(malloc_block);
	    ...
	    if ( gs_malloc_limit - added < malloc_used )
	      msg = "exceeded limit";
		...
		if ( malloc_used > gs_malloc_max )
		  gs_malloc_max = malloc_used;
	      }
	  }
#ifdef DEBUG
	if ( gs_debug_c('a') || msg != ok_msg )
	  dprintf_4("[a+]gs_malloc(%s)(%u) = 0x%lx: %s\n",
...
{	malloc_block *bp = malloc_list;
	...
	for ( ; bp != 0; bp = np )
	   {	np = bp->next;
		if ( gs_debug_c('a') )
		  dprintf_3("[a]gs_malloc_release(%s) 0x%lx(%u)\n",

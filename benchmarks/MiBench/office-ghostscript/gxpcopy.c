 ...
 *	T = (C*U + D*V) / (C^2 + D^2)
 *	(X,Y) = (C*T,D*T)
 * However, any smaller value of T will also work.
 */
private void
adjust_point_to_tangent(segment *pseg, const segment *next,
  const gs_fixed_point *p1)
{	fixed x0 = pseg->pt.x, y0 = pseg->pt.y;
	double C = p1->x - x0, D = p1->y - y0;

	if ( !(C == 0 && D == 0) )	/* anomalous case */
	  { double T = (C * (next->pt.x - x0) + D * (next->pt.y - y0)) * 0.3 /
	      (C * C + D * D);

	    /*
	     * Make sure we don't go off the near end of the line
	     * because of some arithmetical anomaly.
	     * (Going off the far end is OK.)
	     */
	    if_debug3('2', "[2]adjusting: C = %g, D = %g, T = %g\n",
		      C, D, T);
	    if ( T >= 0 )
	      { pseg->pt.x = (fixed)(C * T) + x0;
	        pseg->pt.y = (fixed)(D * T) + y0;
	      }
	  }
}
...
{	dprintf_9("%s p0=(%g,%g) p1=(%g,%g) p2=(%g,%g) p3=(%g,%g)\n",
  ...
  ((fixed)(da * t3d + db * t2d + dc))
	  ...
	  if ( t <= prc->fixed_limit )
		...
		if ( any_abs(xlf - xl) > fixed_epsilon ||
		  ...
		  dprintf_9("Curve points differ: k=%d t=%d a,b,c=%g,%g,%g\n   xl,xd fixed=%g,%g floating=%g,%g\n",
			   k, t,
	...
	if ( gs_debug_c('2') )
	  { int pi;
	    ...
	    if ( nseg == 1 )
	      ...
	      { dprintf_1("[2]Split into %d segments:\n", nseg);

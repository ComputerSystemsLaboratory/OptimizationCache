...
if ( gs_debug_c('o') )
   {	int count = pgs_lp->dash.pattern_size;
	int i;
	dprintf_3("[o]half_width=%f, cap=%d, join=%d,\n",
		 pgs_lp->half_width, (int)pgs_lp->cap, (int)pgs_lp->join);
	dprintf_2("   miter_limit=%f, miter_check=%f,\n",
		 pgs_lp->miter_limit, pgs_lp->miter_check);
	dprintf_1("   dash pattern=%d", count);
	for ( i = 0; i < count; i++ )
	  dprintf_1(",%f", pgs_lp->dash.pattern[i]);
...
{	fixed dx, dy, wx = plp->width.x, wy = plp->width.y;
	...
	/*
	 * If the line is more horizontal, then if the vertical width is
	 * at least 0.5 * sqrt(2), the line is not thin; similarly with
	 * the axes interchanged.
	 *
	 * If both horizontal and vertical widths are less than
	 * 0.5, the line is thin.
	 */
	if ( any_abs(dy) > any_abs(dx) )
	  { fixed awx = any_abs(wx);
	    if ( awx >= float2fixed(0.7071) )
	      return false;
	    if ( awx < fixed_half && any_abs(wy) < fixed_half )
	      return true;
	  }
	else
	  { fixed awy = any_abs(wy);
	    if ( awy >= float2fixed(0.7071) )
	      return false;
	    if ( awy < fixed_half && any_abs(wx) < fixed_half )
	      return true;
	  }
...
if ( gs_debug_c('O') )
   {	dprintf_4("[o]Intersect %f,%f(%f/%f)",
	...
	dprintf_3("\txdiff=%f ydiff=%f denom=%f ->\n",
		 xdiff, ydiff, denom);
   }
#endif
...
#ifdef DEBUG
if ( gs_debug_c('O') )
                   {	dprintf_4("[o]Miter check: u1/v1=%f/%f, u2/v2=%f/%f,\n",
				 u1, v1, u2, v2);
			dprintf_3("        num=%f, denom=%f, check=%f\n",
				 num, denom, check);
                   }
#endif
		    /*

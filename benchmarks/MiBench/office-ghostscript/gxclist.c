...
{	gx_device *target = cdev->target;
	...
	if ( gs_debug_c('l') | gs_debug_c(':') )
	  dprintf_4("[l]width=%d, band_width=%d, band_height=%d, nbands=%d\n",
	   ...
	   {	const char **sub = cmd_sub_op_names[ci >> 4];
		if ( sub != 0 )
		  ...
		  { ulong tcounts = 0, tsizes = 0;
			...
			dprintf_2(" %lu(%lu)", cmd_op_counts[cj],
				 cmd_op_sizes[cj]);
		  }
		dputs("\n");
	   }
...
#ifdef DEBUG
	if ( gs_debug_c('l') | gs_debug_c(':') )
	  dprintf_2("[l]clist_render_init at cfile=%ld, bfile=%ld\n",

...
{	dprintf_4("%s: dev=0x%lx(%s) depth=%d\n",
		 cname, (ulong)dev, dev->dname, dev->color_info.depth);
	dprintf_4("  source data=0x%lx x=%d raster=%u id=%lu colors=",
	...
	if ( scolors )
	  ...
	  dputs("none;\n");
	if ( textures )
	  dprintf_8("  textures=0x%lx size=%dx%d(%dx%d) raster=%u shift=%d(%d)",
	  ...
	  dputs("  textures=none");
	if ( tcolors )
	  dprintf_2(" colors=(%lu,%lu)\n", tcolors[0], tcolors[1]);
	else
	  dputs(" colors=none\n");
	dprintf_7("  rect=(%d,%d),(%d,%d) phase=(%d,%d) op=0x%x\n",

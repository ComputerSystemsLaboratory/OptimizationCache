...
#ifdef DEBUG
if ( gs_debug_c('x') )
	dprintf_("[x]setting char_tm:"), trace_matrix_fixed(&pgs->char_tm);
#endif
...
#ifdef DEBUG
if ( gs_debug_c('x') )
	dprintf_2("[x]translate_to_fixed %g, %g:\n",
...
#ifdef DEBUG
if ( gs_debug_c('x') )
   {	dprintf_6("[x]ctm: [%6g %6g %6g %6g %6g %6g]\n",
		 ctm.xx, ctm.xy, ctm.yx, ctm.yy, ctm.tx, ctm.ty);
	dprintf_6("   scale=%d fc: [0x%lx 0x%lx 0x%lx 0x%lx] shift=%d\n",

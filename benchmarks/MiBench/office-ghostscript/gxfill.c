...
{	const active_line *lp;
	for ( lp = flp; lp != 0; lp = lp->next )
	   {	fixed xc = lp->x_current, xn = lp->x_next;
		dprintf_3("[f]0x%lx(%d): x_current/next=%g",
		         (ulong)lp, lp->direction,
		         fixed2float(xc));
		if ( xn != xc ) dprintf_1("/%g", fixed2float(xn));
		dputc('\n');
	   }
}
...
#ifdef DEBUG
		if ( gs_debug_c('F') )
		  {	dprintf_2("[F]before loop: y=%f y1=%f:\n",

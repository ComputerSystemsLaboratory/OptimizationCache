...
#ifdef DEBUG
		if ( gs_debug_c('a') )
		  {	dprintf_1("[a%d]", mem->space);
			dprintf__chunk("closing chunk", mem->pcc);
		  }
#endif
	}
}
...
{	if ( mem->pcc != 0 )
	{	mem->cc = *mem->pcc;
		...
		if ( gs_debug_c('a') )
		  {	dprintf_1("[a%d]", mem->space);
			dprintf__chunk("opening chunk", mem->pcc);
		  }
#endif
	}
}
...
{	const obj_header_t *pre = ((const obj_header_t *)obj) - 1;
	...
	dprintf_3("  pre=0x%lx(obj=0x%lx) size=%lu", (ulong)pre, (ulong)obj,
		 size);
	switch ( options & (dump_do_type_addresses | dump_do_no_types) )
	  {
	  case dump_do_type_addresses + dump_do_no_types: /* addresses only */
	    dprintf_1(" type=0x%lx", (ulong)type);
	    break;
	  case dump_do_type_addresses: /* addresses & names */
	    dprintf_2(" type=%s(0x%lx)", struct_type_name_string(type),
		     (ulong)type);
	    break;
	  case 0:		/* names only */
	    dprintf_1(" type=%s", struct_type_name_string(type));
	  case dump_do_no_types: /* nothing */
	    ;
	  }
	if ( options & dump_do_marks ) {
	  if ( pre->o_large )
	    dprintf_1(" lmark=%d", pre->o_lmark);
	  else
	    dprintf_2(" smark/back=%u (0x%x)", pre->o_smark, pre->o_smark);
	}
	dputc('\n');
	if ( type == &st_free )
	  return;
	if ( options & dump_do_pointers ) {
	  ...
	  if ( proc != 0 )
		...
		dprintf_1((ptr_between(ptr, obj, (byte *)obj + size) ?
			  "(0x%lx)\n" : "0x%lx\n"), (ulong)ptr);
	      }
	    }
	}
...
{	dprintf_1("chunk at 0x%lx:\n", (ulong)cp);
	dprintf_3("   chead=0x%lx  cbase=0x%lx sbase=0x%lx\n",
		 (ulong)cp->chead, (ulong)cp->cbase, (ulong)cp->sbase);
	dprintf_3("    rcur=0x%lx   rtop=0x%lx  cbot=0x%lx\n",
		 (ulong)cp->rcur, (ulong)cp->rtop, (ulong)cp->cbot);
	dprintf_4("    ctop=0x%lx climit=0x%lx smark=0x%lx, size=%u\n",
		 (ulong)cp->ctop, (ulong)cp->climit, (ulong)cp->smark,
		 cp->smark_size);
	dprintf_2("  sreloc=0x%lx   cend=0x%lx\n",
		 (ulong)cp->sreloc, (ulong)cp->cend);
	dprintf_5("cprev=0x%lx cnext=0x%lx outer=0x%lx inner_count=%u has_refs=%s\n",
		 (ulong)cp->cprev, (ulong)cp->cnext, (ulong)cp->outer,
		 cp->inner_count, (cp->has_refs? "true" : "false"));

	dprintf_2("  sfree1=0x%lx   sfree=0x%x\n",

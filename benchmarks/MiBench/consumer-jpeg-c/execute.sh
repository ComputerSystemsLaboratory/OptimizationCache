...
function verify_output() {
    # Diff the two files.
    diff reference_output/reference_output_$WORKING_SET.ppm output.ppm > diff.txt 2>&1
    if [[ $? -eq 0 ]]; then
        # They are igual
...
case $WORKING_SET in
    0)
        RUN_OPTIONS="-dct int -progressive -opt -outfile output.ppm input_small.ppm"
        ;;
    1)
        RUN_OPTIONS="-dct int -progressive -opt -outfile output.ppm input_large.ppm"
        ;;
    *)
        echo "Error: dataset"
        exit 1

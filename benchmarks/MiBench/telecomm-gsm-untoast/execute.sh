...
function verify_output() {
    # Diff the two files.
    diff reference_output/reference_output_$WORKING_SET.gsm $OUTPUT > diff.txt 2>&1
    if [[ $? -eq 0 ]]; then
        # They are igual
...
case $WORKING_SET in
    0)
        RUN_OPTIONS="-fps small.au.run"
	OUTPUT="small.au.run.gsm"
        ;;
    1)
        RUN_OPTIONS="-fps large.au.run"
	OUTPUT="large.au.run.gsm"
        ;;
    *)
        echo "Error: dataset"
        exit 1

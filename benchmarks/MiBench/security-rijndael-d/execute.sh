...
function verify_output() {
    # Diff the two files.
    diff reference_output/reference_output_$WORKING_SET.dec output.dec > diff.txt 2>&1
    if [[ $? -eq 0 ]]; then
        # They are igual
...
case $WORKING_SET in
    0)
        RUN_OPTIONS="input_small.enc output.dec d 1234567890abcdeffedcba09876543211234567890abcdeffedcba0987654321"
  	;;
    1)
        RUN_OPTIONS="input_large.enc output.dec d 1234567890abcdeffedcba09876543211234567890abcdeffedcba0987654321"
        ;;
    *)
        echo "Error: dataset"
        exit 1
